...
# Load all dataframes
agg_trans_df = load_data('aggregated_transactions')
agg_user_df = load_data('aggregated_users')
map_trans_df = load_data('map_transactions')
map_user_df = load_data('map_users')
top_trans_df = load_data('top_transactions')
top_user_df = load_data('top_users')
...
title_text = '''PHONEPE PULSE: PhonePe Transaction Insights'''
...
if selected == "ABOUT":
        ...
        st.markdown('Github Cloning, Python, Pandas, MySQL,mysql-connector-python, Streamlit, and Plotly.',unsafe_allow_html=True)
        st.subheader(':violet[Overview:]')
        st.markdown('''
                Git: Employed Git for version control and efficient collaboration, enabling seamless cloning of the PhonePe dataset from GitHub.
                Pandas: Leveraged the powerful Pandas library to transform the dataset from JSON format into a structured dataframe.
                Pandas facilitated data manipulation, cleaning, and preprocessing, ensuring the data was ready for analysis.
                SQL Alchemy: Utilized SQL Alchemy to establish a connection to a SQL database, enabling seamless integration of the transformed dataset
                ...
                Plotly: Integrated Plotly, a versatile plotting library, to generate insightful visualizations from the dataset. Plotly's interactive plots,
...
if selected =="HOME":
        ...
        with col1:
                ...
                st.markdown('''PhonePe is a popular digital payments platform in India, offering a range of financial services through its mobile app.
                        ...
                        PhonePe has become a preferred choice for millions of users, contributing to India's digital payments revolution.''',unsafe_allow_html=True)

                st.subheader(':violet[what is Phonepe Pulse?]')
                st.markdown('''PhonePe Pulse provides real-time insights and trends on digital payments across India.
        ...
        with col2:
                st.markdown("![Alt Text](https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExY2Vvb2oyaHBqNHZzdm9ycG5lcDEyczk3dDZtcnplamdpbGJudG0xNiZlcD12MV9iZW5jZG9uZ19ncm91bmQmc3ludGhlc2lzPXRydWU/gSU4qpRY00OOe6v8I8/giphy-downsized-large.gif)")

        col1,col2=st.columns(2)
        with col1:
                st.image('https://www.phonepe.com/pulsestatic/791/pulse/static/4cb2e7589c30e73dca3d569aea9ca280/1b2a8/pulse-2.webp',use_container_width=True)
        with col2:
                st.write(' ')
                st.subheader(':violet[Discover Insights:]')
                st.markdown('''
                        Transaction:Transaction insights involve analyzing customer transaction data to understand behavior and preferences.
                        By examining trends, categorizing transactions,and identifying patterns of india.
                        User: User insights refer to analyzing customer demographics, engagement metrics, and feedback.
...
if selected =="GEO VISUALIZATION":
        ...
        st.write("You selected:", f"{geo_type}", unsafe_allow_html=True)
        ...
        if geo_type=="**Transactions**":
                ...
                if not trans_geo_year_wise:
                        ...
                        st.write("You selected:", f"{cat}", unsafe_allow_html=True)
                        ...
                        if cat =="Transaction Amount":
                                ...
                                fig = px.choropleth_mapbox(df,geojson=ind_geo(),featureidkey='properties.ST_NM',
                                        locations='state',
                                        hover_data=['Transaction Amount','Average Transaction Amount'],
                        ...
                        if cat =="Transaction Count":
                                ...
                                df = pd.read_sql_query('''SELECT state,sum(Transaction_count) as 'Transaction Count',
                ...
                if trans_geo_year_wise:
                        df_year=pd.read_sql_query('''SELECT DISTINCT year as 'Year' from aggregated_transactions''',con=engine)
                        ...
                        if not trans_geo_quater_wise:
                                df = pd.read_sql_query('''SELECT state,sum(Transaction_amount) as 'Transaction Amount',
                                                ...
                                                AVG(Transaction_count) as 'Average Transaction Count'
                                                from aggregated_transactions where year=?
                                                GROUP by state''',con=engine,params=(selected_year,))

                                fig = px.choropleth_mapbox(df,geojson=ind_geo(),featureidkey='properties.ST_NM',
                                        locations='state',
                                        hover_data=['Transaction Amount','Average Transaction Amount','Transaction Count','Average Transaction Count'],
                                        color='Transaction Amount',
                        ...
                        if trans_geo_quater_wise:
                                df_quater=pd.read_sql_query('''SELECT DISTINCT Quater as 'Quater' from aggregated_transactions''',con=engine)
                                ...
                                df = pd.read_sql_query('''SELECT state,sum(Transaction_amount) as 'Transaction Amount',
                                                ...
                                                sum(Transaction_count) as 'Transaction Count',
                                                AVG(Transaction_count) as 'Average Transaction Count'
                                                from aggregated_transactions where year=? and Quater=?
        ...
        if geo_type=="**Users**":
                ...
                if user_geo_year_wise:
                        df_year=pd.read_sql_query('''SELECT DISTINCT year as 'Year' from map_users''',con=engine)
                        selected_year = st.select_slider("Select Year",options=df_year['Year'].tolist())
                        ...
                        if user_geo_quater_wise:
                                df_quater=pd.read_sql_query('''SELECT DISTINCT Quater as 'Quater' from map_users''',con=engine)
                                selected_Quater = st.select_slider("Select Quater",options=df_quater['Quater'].tolist())

                                df = pd.read_sql_query('''SELECT DISTINCT state, SUM(Registered_Users) as 'Total Registered User',
                                                AVG(Registered_Users) as 'Average Register User'
                                                FROM map_users WHERE  year=? and Quater=?
                                                GROUP BY state''',con=engine,params=(selected_year,selected_Quater))
...
if selected =="INSIGHTS":
        ...
        if select_insight =="TOP INSIGHTS":
                ...
                if query=='Top 10 states with highest transaction':
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT state, SUM(Transaction_count) AS 'Transaction Count' FROM aggregated_transactions
                                                WHERE state IN ( SELECT state FROM
                                                (SELECT state, SUM(Transaction_amount) AS amount FROM aggregated_transactions
                ...
                if query=='Top 10 states with highest Registered User':
                        col1,col2=st.columns(2)
                        with col1:
                                df=pd.read_sql_query('''SELECT state ,sum(Registered_Users) as 'Registered User' FROM map_users
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT state, SUM(App_Opens) AS 'App Opened' FROM map_users WHERE state IN
                                                (SELECT state  FROM (SELECT state, SUM(Registered_Users) AS 'R_user'
                                                FROM map_users GROUP BY state ORDER BY sum(Registered_Users) DESC LIMIT 10)as top_user )
                        ...
                        t_1='''🟡 Maharashtra tops the list with the highest number of PhonePe registered users,
                        indicating a strong adoption of digital payment platforms in the state's urban and rural areas.'''
                ...
                if query=='Top 10 District with highest transaction':
                        ...
                        with col1:
                                df=pd.read_sql_query('''SELECT District,sum(Transaction_amount) as 'Transaction Amount'
                                        from map_transactions GROUP by District
                                        ORDER by sum(Transaction_amount) DESC LIMIT 10;''',con=engine)
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT District, SUM(Transaction_count) AS 'Transaction Count' FROM map_transactions
                                        WHERE District IN (SELECT District  FROM (SELECT District, SUM(Transaction_amount) AS 'amount'
                                        FROM map_transactions GROUP BY District ORDER BY amount DESC LIMIT 10)as top_dist )
                ...
                if query=='Top 10 District with lowest transaction':
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT District, SUM(Transaction_count) AS 'Transaction Count' FROM map_transactions
                                        WHERE District IN (SELECT District  FROM (SELECT District, SUM(Transaction_amount) AS 'amount'
                                        FROM map_transactions GROUP BY District ORDER BY amount ASC LIMIT 10)as top_dist )
                ...
                if query=='Top 10 Brands used for Transaction':
                        ...
                        with col1:
                                df=pd.read_sql_query('''SELECT DISTINCT User_brand as 'User Brand' ,SUM(User_count) as 'Count'
                                                FROM aggregated_users GROUP BY User_brand
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT User_brand AS 'User Brand', (SUM(User_count) / total_count) * 100 AS 'Percentage'
                                                ...
                                                CROSS JOIN (SELECT SUM(User_count) AS total_count FROM aggregated_users) AS total
                                                GROUP BY User_brand
                                                ORDER BY SUM(User_count) DESC LIMIT 10;''',con=engine)
                ...
                if query=='Top 10 District with highest Registered User':
                        col1,col2=st.columns(2)
                        with col1:
                                df=pd.read_sql_query('''SELECT District, sum(Registered_Users) as 'Registered User' FROM map_users
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT District, SUM(App_Opens) AS 'App Opened' FROM map_users WHERE District IN
                                                (SELECT District  FROM (SELECT District, SUM(Registered_Users) AS 'R_user'
                                                FROM map_users GROUP BY District ORDER BY sum(Registered_Users) DESC LIMIT 10)as top_user )
                ...
                if query=='Sum of Transaction by Type or categories':
                        ...
                        with col1:
                                df=pd.read_sql_query('''SELECT DISTINCT Transaction_type as 'categories',SUM(Transaction_amount) as 'Transaction Amount'
                                                        from aggregated_transactions GROUP BY Transaction_type DESC''',con=engine)
                        ...
                        highlighting the importance of banking and investment activities.'''
                ...
                if query=='Top 10 Postal code with highest Transaction':
                        col1,col2=st.columns(2)
                        with col1:
                                df=pd.read_sql_query('''SELECT Pincode, sum(Transaction_amount) as 'Transaction Amount' FROM top_transactions
                        ...
                        with col2:
                                df=pd.read_sql_query('''SELECT Pincode , SUM(Transaction_count) AS 'Transaction Count' FROM top_transactions
                                                WHERE Pincode IN (SELECT Pincode FROM (SELECT Pincode, SUM(Transaction_amount) AS 't_amt'
                                                FROM top_transactions GROUP BY Pincode ORDER BY SUM(Transaction_amount) DESC LIMIT 10)as top_tran )
                ...
                if query=='Top 10 Postal code with highest Registered user':
                        col1,col2=st.columns(2)
                        with col1:
                                df=pd.read_sql_query('''SELECT Pincode, sum(Registered_Users) as 'Registered user' FROM top_users
        ...
        if select_insight =="FILTER INSIGHTS":
                ...
                st.write("You selected:", f"{fil_type}", unsafe_allow_html=True)
                ...
                if fil_type=="**State**":
                        ...
                        if Query==ques[1]:
                                df_state=pd.read_sql_query('''Select DISTINCT state from aggregated_transactions''',con=engine)
                                ...
                                if bt:
                                        ...
                                        with col1:
                                                df=pd.read_sql_query(''' SELECT Quater,sum(Transaction_amount) as 'Transaction Amount'
                                                                from aggregated_transactions WHERE state=? and year=?
                        ...
                        if Query==ques[3]:
                                ...
                                df_type=pd.read_sql_query('''SELECT DISTINCT Transaction_type as 'Type' from aggregated_transactions''',con=engine)
                                select_type=st.selectbox('Select Type',options=df_type['Type'].tolist(),index=None)
                                bt=st.button('Show')
                                if bt:
                                        col1,col2=st.columns(2)
                                        with col1:
                                                df=pd.read_sql_query('''SELECT state,Quater,sum(Transaction_amount) as 'Transaction Amount' from aggregated_transactions
                                                                where state =? and Transaction_type=? group by state,Quater''',con=engine,params=(select_state,select_type))
                                        ...
                                        with col2:
                                                df=pd.read_sql_query('''SELECT state,Quater,sum(Transaction_count) as 'Transaction Count' from aggregated_transactions
                                                                where state =? and Transaction_type=? group by state,Quater''',con=engine,params=(select_state,select_type))
                        ...
                        if Query==ques[4]:
                                df_state=pd.read_sql_query('''Select DISTINCT state from aggregated_users''',con=engine)
                                select_state=st.selectbox('Select state',options=df_state['state'].tolist(),index=None)

                                df_year=pd.read_sql_query('''SELECT DISTINCT year as 'Year' from aggregated_users''',con=engine)
                                select_year = st.selectbox("Select Year",options=df_year['Year'].tolist(),index=None)
                                bt=st.button('Show')
                                if bt:
                                        col1,col2=st.columns(2)
                                        with col1:
                                                df=pd.read_sql_query('''SELECT User_brand as 'User Brand',sum(User_count) as 'Count' from aggregated_users
                        ...
                        if Query==ques[5]:
                                df_state=pd.read_sql_query('''Select DISTINCT state from map_users''',con=engine)
                                select_state=st.selectbox('Select state',options=df_state['state'].tolist(),index=None)
                                ...
                                if bt:
                                        col1,col2=st.columns(2)
                                        with col1:
                                                df=pd.read_sql_query('''SELECT Quater,sum(Registered_Users) as 'Registered Users' from map_users
                                                                WHERE state =? and year=?
                                                                GROUP by Quater''',con=engine,params=(select_state,select_year))
                                        ...
                                        with col2:
                                                df=pd.read_sql_query('''SELECT Quater,sum(App_Opens) as 'App opened' from map_users
                                                                WHERE state =? and District= ?
                                                                GROUP by Quater''',con=engine,params=(select_state,select_dist))
                ...
                if fil_type=="**District**":
                        ...
                        if Query==ques[0]:
                                df_state=pd.read_sql_query('''SELECT DISTINCT state from map_transactions''',con=engine)
                                select_state=st.selectbox('Select state',options=df_state['state'].tolist(),index=None)

                                df_year=pd.read_sql_query('''SELECT DISTINCT year from map_transactions''',con=engine)
                                select_year = st.selectbox("Select Year",options=df_year['year'].tolist(),index=None)
                                bt=st.button('Show')
                                if bt:
                                        df=pd.read_sql_query('''SELECT District, SUM(Transaction_amount) as 'Transaction Amount'
                                        FROM map_transactions where state=? and  year =?
                        ...
                        if Query==ques[1]:
                                ...
                                df_dist=pd.read_sql_query('''SELECT DISTINCT District FROM map_transactions
                                                        where state=?''',con=engine,params=(select_state,))
                                select_dist=st.selectbox('Select District',options=df_dist['District'].tolist(),index=None)
                                bt=st.button('Show')
                                if bt:
                                        col1,col2=st.columns(2)
                                        with col1:
                                                df=pd.read_sql_query('''SELECT year,sum(Transaction_amount) as 'Transaction Amount' from map_transactions
                                                        where state=? and District= ?
                                        ...
                                        with col2:
                                                df=pd.read_sql_query('''SELECT year,sum(Transaction_count) as 'Transaction count' from map_transactions
                                                        where state=? and District= ?
                                                        GROUP by year''',con=engine,params=(select_state,select_dist))
                        ...
                        if Query==ques[2]:
                                ...
                                df_dist=pd.read_sql_query('''SELECT DISTINCT District FROM map_users
                                                        where state=?''',con=engine,params=(select_state,))
                                select_dist=st.selectbox('Select District',options=df_dist['District'].tolist(),index=None)
                                bt=st.button('Show')
                                if bt:
                                        col1,col2=st.columns(2)
                                        with col1:
                                                df=pd.read_sql_query('''SELECT year,sum(Registered_Users) as 'Registered User' from map_users
                                                        where state=? and District= ?
                                                        GROUP by year''',con=engine,params=(select_state,select_dist))
                                        ...
                                        with col2:
                                                df=pd.read_sql_query('''SELECT year,sum(App_Opens) as 'App opened' from map_users
                                                        where state=? and District= ?
                                                        GROUP by year''',con=engine,params=(select_state,select_dist))
